...
(defcustom org-ews-host nil
  "The address of the targeted Exchange server."
  :type 'string
  :group 'org-ews)

(defcustom org-ews-auth-mechanism :any
  "The auth mechanism used by `org-ews' to authenticate to the Exchange server.
Valid values include:
  `:any' - Let cURL figure out the authentication method
  `:basic' - Use HTTP basic authentication
  `:digest' - Use HTTP digest authentication
  `:ntlm' - Use Microsoft's NTLM authentication
  `:negotiate' - Use Negotiate (SPNEGO) authentication"
  :group 'org-ews)

(defcustom org-ews-curl-executable "curl"
  "The path to the cURL executable on this system."
  :type 'string
  :group 'org-ews)

(defconst org-ews--curl-charset "utf-8"
  "The charset used by cURL requests.")
